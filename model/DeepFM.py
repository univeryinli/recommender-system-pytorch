...
class DeepFM(torch.nn.Module):
    ...
    is_batch_norm锛歜ool,  use batch_norm or not ?
    verbose: verbose
    ...
    def __init__(self, field_size, feature_sizes, embedding_size=4, is_shallow_dropout=True, dropout_shallow=[0.5, 0.5],
                 h_depth=2, deep_layers=[64, 64], is_deep_dropout=True, dropout_deep=[0.5, 0.5, 0.5],
                 deep_layers_activation='relu', n_epochs=24, batch_size=256, learning_rate=0.003,
    ...
    def fit(self, Xi_train, Xv_train, y_train, Xi_valid=None, Xv_valid=None,
        ...
        for epoch in range(self.n_epochs):
            ...
            for i in range(batch_iter + 1):
                ...
                if self.verbose:
                    if i % 100 == 99:  # print every 100 mini-batches
                        pre=outputs.cpu().data.numpy()
                        y=batch_y.cpu().data.numpy()
                        roc=self.roc(y,pre)
                        print('[epoch,batch]: [%d, %5d] train_loss: %.6f train_metric: %.6f time: %.1f s' %
                              (epoch + 1, i + 1, total_loss / 100.0, roc, time() - batch_begin_time))
                        total_loss = 0.0
                        batch_begin_time = time()
                else:
                    #pre = outputs.cpu().data.numpy()
                    ...
                    roc=-1
                    print('[epoch,batch]: [%d, %5d] train_loss: %.6f train_metric: %.6f time: %.1f s' %
                          (epoch + 1, i + 1, loss.item(), roc, time() - batch_begin_time))
                    batch_begin_time = time()

            train_loss, train_eval = self.eval_by_batch(Xi_train, Xv_train, y_train, x_size)
            train_result.append(train_eval)
            print('*' * 50)
            print('epoch_result:[%d] train_loss: %.6f train_metric: %.6f time: %.1f s' %
            ...
            if is_valid:
                ...
                print('*' * 50)
                print('epoch_result:[%d] val_loss: %.6f val_metric: %.6f time: %.1f s' %
    ...
    def eval_by_batch(self, Xi, Xv, y, x_size):
        ...
        for i in range(batch_iter + 1):
            ...
            y1.extend(batch_y.cpu().data.numpy())
            total_loss += loss.item()
        roc=self.roc(y1,y_pred)
        return total_loss / 10, roc
    ...
    def predict_from_model_file(self, Xi, Xv,model,path):
        """
        ...
        pred = F.sigmoid(model(Xi, Xv)).cpu().data.numpy()
        return pred
    ...
    def roc(self, y,pre):
        ...
        total_metric=0
        if len(set(y)) == 2:
            total_metric = roc_auc_score(y, pre)
        else:
            total_metric = -1
        return total_metric
