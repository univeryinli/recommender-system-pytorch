...
def read_criteo_data(file_path,emb_file):
    result = {'lable':[], 'index':[],'value':[],'feature_sizes':[]}
    ...
    for line in f:
        datas = line.strip().split(',')
        result['lable'].append(int(datas[0]))
...
def read_csv_dataset(train_csv,task='like'):
    ...
    if task=='like':
        lable=train_csv[task]
    elif task=='finish':
        lable=train_csv[task]
    train_dict['lable']=lable[0:int(len(lable)*0.8)].to_list()
    test_dict['lable']=lable[int(len(lable)*0.8)+1:-1].to_list()

    feild = ['uid','user_city','item_id','author_id','item_city','channel','music_id','video_duration']
    value=[1]*len(feild)
    values=[value for i in range(len(lable))]
    train_dict['value']=values[0:int(len(lable)*0.8)]
    test_dict['value']=values[int(len(lable)*0.8)+1:-1]
    ...
    train_dict['index']=temp[0:int(len(lable)*0.8)].tolist()
    test_dict['index']=temp[int(len(lable)*0.8)+1:-1].tolist()
...
def read_csv_dataset_pred(pred_csv,task='like'):
    ...
    if task=='like':
        ...
        lable=train_csv[task]
    pred_dict['lable']=lable.to_list()

    feild = ['uid','user_city','item_id','author_id','item_city','channel','music_id','creat_time','video_duration']
    value=[1]*len(feild)
    values=[value for i in range(len(lable))]
